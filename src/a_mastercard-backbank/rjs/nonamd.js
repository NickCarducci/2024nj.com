 ...
 *            deps: { // instead of just an array, an object allows defining the defi function argument names
 ...
 *     }
 *   }
 * }); AST or eval is the issue, AMD is the solution
 * ``` problem is assumptions made by current failing or incomplete solutions
 */
const defi = (x, cb) => cb(...x);
defi(["module", "text"], function (module, text) {
   ...
   *    the first line of the module's defi function.
   * @param {string} [moduleConfig.postscript] Arbitrary JavaScript code placed at
   *    the end of the module's defi function.
   ...
   *    having as keys the module's defi function argument names and as values the correposding module ids.
  ...
  function compileNonAmd(jsText, moduleConfig, isBuild) {
    ...
    jsText =
      "defi(" +
      compileHeader(moduleConfig) +
      "  var defi = undefined;\n" + // Avoid 'defi' detection and self registration
      (prescript ? "  " + prescript + "\n" : "") +
      jsText +
      "\n" +
  ...
  // The text following "defi(", until the opening brace, inclusive.
