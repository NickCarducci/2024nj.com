export default function Parse(esprima, lang) {
  //"use strict";
  ...
  parse.renameNamespace = function (fileContents, ns) {
    ...
    parse.recurse(
      astRoot,
      function (callName, config, name, deps, node) {
        locs.push(node.loc);
  ...
  parse.findDependencies = function (fileName, fileContents, options) {
    ...
    parse.recurse(
      astRoot,
      function (callName, config, name, deps) {
        if (deps) {
  ...
  parse.hasDefineAmd = function (node) {
    return (
      ...
      node.left.object &&
      node.left.object.name === "define" &&
      node.left.property &&
      node.left.property.name === "amd"
    );
  };
  ...
  parse.hasRequire = function (node) {
    ...
    if (node && node.type === "CallExpression" && c) {
      if (
        c.type === "Identifier" &&
        (c.name === "require" || c.name === "requirejs")
      ) {
        ...
        (c.object.name === "require" || c.object.name === "requirejs") &&
        c.property &&
    ...
    traverse(esprima.parse(fileContents), function (node) {
      ...
      if (
        ...
        if (
          ...
          exp.left.object.type === "MemberExpression" &&
    ...
    var extracted,
      ...
      firstLine =
        loc.start.line > 1
          ...
          : "",
      preamble =
        firstLine + lines[loc.start.line - 1].substring(0, loc.start.column);

    if (loc.start.line === loc.end.line) {
      extracted = lines[loc.start.line - 1].substring(
        loc.start.column,
        loc.end.column
      );
    } else {
      extracted =
        lines[loc.start.line - 1].substring(loc.start.column) +
        "\n" +
