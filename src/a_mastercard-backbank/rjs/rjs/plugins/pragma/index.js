export default function Pragma(parse, logger) {
  //"use strict";
  function Temp() {}
  ...
  var pragma = {
    ...
    removeStrict: function (contents, config) {
      return config.useStrict
        ? contents
        : contents.replace(pragma.useStrictRegExp, "$1");
    },
    ...
    namespace: function (fileContents, ns, onLifecycleName) {
      if (ns) {
        ...
        fileContents = fileContents.replace(
          pragma.defineExistsAndAmdRegExp,
        ...
        //checks above.
        fileContents = fileContents.replace(
          pragma.defineCheckRegExp,
          "typeof " + ns + ".define === 'function' && " + ns + ".define.amd"
        );
        fileContents = fileContents.replace(
          pragma.defineStringCheckRegExp,
          "typeof " + ns + ".define === 'function' && " + ns + ".define['amd']"
        );
        fileContents = fileContents.replace(
          pragma.defineTypeFirstCheckRegExp,
          "'function' === typeof " + ns + ".define && " + ns + ".define.amd"
        );
        fileContents = fileContents.replace(
          pragma.defineExistsRegExp,
        ...
        if (
          ...
          fileContents.indexOf("if (!" + ns + " || !" + ns + ".requirejs)") ===
            -1
        ) {
