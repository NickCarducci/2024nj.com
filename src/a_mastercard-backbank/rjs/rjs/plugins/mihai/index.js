  ...
  return (function () {
      ...
      function collapse_single_use_vars(statements, compressor) {
        ...
        for (var stat_index = statements.length; --stat_index >= 0; ) {
          ...
          for (var var_defs_index = var_defs.length; --var_defs_index >= 0; ) {
            ...
            var value_has_side_effects = var_decl.value.has_side_effects(
              compressor
            );
            ...
            case stat instanceof AST_If:
              if (stat.body instanceof AST_Return) {
                ...
                if (
                  ...
                  i > 0 &&
                  statements[i - 1] instanceof AST_If &&
                  statements[i - 1].body instanceof AST_Return &&
                  ret.length == 1 &&
                  in_lambda &&
    ...
    (function (def) {
      ...
      def(AST_Binary, function (pure_getters) {
        switch (this.operator) {
          ...
          case "||":
            return (
              this.left._throw_on_access(pure_getters) &&
      ...
      def(AST_Assign, function (pure_getters) {
        return (
          this.operator == "=" && this.right._throw_on_access(pure_getters)
        );
      });
      def(AST_Conditional, function (pure_getters) {
        return (
          this.consequent._throw_on_access(pure_getters) ||
    ...
    (function (def) {
      ...
      AST_Node.DEFMETHOD("evaluate", function (compressor) {
        ...
        try {
          ...
          return !val || val instanceof RegExp || typeof val != "object"
            ? val
            : this;
        } catch (ex) {
          if (ex !== def) throw ex;
      ...
      AST_Node.DEFMETHOD("constant_value", function (compressor) {
        ...
        if (
          ...
          switch (this.operator) {
            ...
            default:
              throw new Error(
                string_template("Cannot evaluate unary expression {value}", {
        ...
        throw new Error(
          string_template(
            "Cannot evaluate constant [{file}:{line},{col}]",
            this.start
          )
        );
      });
    ...
    AST_Call.DEFMETHOD("has_pure_annotation", function (compressor) {
      ...
      if (
        ...
        comments.length &&
        /[@#]__PURE__/.test(
          (last_comment = comments[comments.length - 1]).value
        )
      ) {
        pure = last_comment;
      }
    ...
    (function (def) {
      ...
      def(AST_Call, function (compressor) {
        if (
          !this.has_pure_annotation(compressor) &&
      ...
      if (
        ...
        var tt = new TreeTransformer(function before(node, descend, in_list) {
          ...
          if (
            ...
            !(tt.parent() instanceof AST_ForIn && tt.parent().init === node)
          ) {
            var def = node.definitions.filter(function (def) {
              ...
              if (
                def.value &&
                (def._unused_side_effects = def.value.drop_side_effect_free(
                  compressor
                ))
              ) {
                compressor.warn(
      ...
      def(AST_Call, function (compressor, first_in_statement) {
        if (
          ...
          if (
            ...
            var node = this.clone();
            node.expression = node.expression.process_expression(
              false,
              compressor
            );
            return node;
    ...
    OPT(AST_DWLoop, function (self, compressor) {
      ...
      if (cond !== self.condition) {
        ...
        if (compressor.option("dead_code") && self instanceof AST_While) {
          ...
          return make_node(AST_BlockStatement, self, { body: a }).optimize(
            compressor
          );
        }
    ...
    OPT(AST_Switch, function (self, compressor) {
      ...
      if (value !== self.expression) {
        var expression = make_node_from_constant(
          value,
          self.expression
        ).transform(compressor);
    ...
    OPT(AST_Call, function (self, compressor) {
      ...
      if (compressor.option("unsafe")) {
        if (exp instanceof AST_SymbolRef && exp.undeclared()) {
          ...
          exp instanceof AST_Dot &&
          exp.expression.is_string(compressor) &&
          exp.property == "charAt"
        ) {
          var arg = self.args[0];
    ...
    OPT(AST_Seq, function (self, compressor) {
      ...
      self.car = self.car.drop_side_effect_free(
        compressor,
        ...
        if (
          ...
          while (true) {
            if (cdr.equivalent_to(left)) {
              var car =
                self.car instanceof AST_UnaryPostfix
                  ? make_node(AST_UnaryPrefix, self.car, {
          ...
          case "&&":
            ...
            if (
              ...
              if (!rr) {
                ...
                compressor.warn(
                  "Dropping side-effect-free && in boolean context [{file}:{line},{col}]",
                  self.start
                );
            ...
            if (
              ...
              self = make_node(AST_Binary, self, {
                ...
                right: make_node(AST_String, self.right, {
                  value:
                    "" + self.left.right.getValue() + self.right.getValue(),
      ...
      if (
        ...
        if (fixed) {
          if (d.should_replace === undefined) {
            ...
            if (
              init !== fixed &&
              (compressor.option("unsafe_regexp") || !(init instanceof RegExp))
            ) {
              ...
              if (
                compressor.option("unused") &&
                (!d.global || compressor.option("toplevel"))
              ) {
    ...
    OPT(AST_VarDef, function (self, compressor) {
      ...
      if (defines && HOP(defines, self.name.name)) {
        compressor.warn(
          "global_defs " + self.name.name + " redefined [{file}:{line},{col}]",
          self.start
        );
      }
      return self;
