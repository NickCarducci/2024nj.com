  ...
  try {
    ...
    try {
      ...
      try {
        ...
        JSSourceFilefromCode = Java.type(
          "com.google.javascript.jscomp.SourceFile"
        ).class.getMethod("fromCode", [stringClass, stringClass]);
      } catch (e) {}
    }
  }
  ...
  optimize = {
    closure: function (
      ...
      var result,
        ...
        CommandLineRunner =
          Packages.com.google.javascript.jscomp.CommandLineRunner;
      ...
      if (config.generateSourceMaps) {
        ...
        mappings.add(
          new com.google.javascript.jscomp.SourceMap.LocationMapping(
            fileName,
            baseName + ".src.js"
          )
        );
      ...
      if (result.success) {
        ...
        if (
          ...
          if (file.exists(outFileNameMap)) {
            concatNameMap = outFileNameMap.replace(
              /\.map$/,
              ".src.js.map"
            );
          ...
          file.saveFile(
            outFileNameMap,
            file
              .readFile(outFileNameMap)
              .replace(mapRegExp, '"file":"' + baseName + '"')
          );
