  ...
  return (function () {
    ...
    req.get = function (context, moduleName, relModuleMap, localRequire) {
      if (
        ...
        context.onError(
          makeError(
            "Explicit require of " + moduleName + " is not allowed.",
            moduleName
          )
        );
      }
    ...
    req.load = function (context, moduleName, url) {
      ...
      if (
        ...
        (!config.suppress || !config.suppress.nodeShim)
      ) {
        console.warn(
          "Shim config not supported in Node, may or may not work. Detected " +
            "for module: " +
            moduleName
        );
      }
      ...
      if (exists(url)) {
        ...
        def(moduleName, function () {
          ...
          var dirName,
            map =
              hasProp(context.registry, moduleName) &&
